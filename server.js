...
app.get('/api/alertas', async (req, res) => {
    if (cacheAlertas.data && (Date.now() - cacheAlertas.lastUpdate < 30000)) return res.json(cacheAlertas.data);
    try {
        ...
        cacheAlertas = { data: { inmet, noaa }, lastUpdate: Date.now() };
...
app.get('/api/clima-clique', async (req, res) => {
    ...
    try {
        const [clima, geo] = await Promise.allSettled([
            axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,weather_code,pressure_msl,visibility,wind_speed_10m,wind_direction_10m&timezone=auto`),
