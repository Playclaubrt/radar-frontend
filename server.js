...
const ALERTAS_TTL = 30000; // 30 segundos

let cacheClima = new Map();
const CLIMA_TTL = 10000; // 10 segundos

app.get('/api/alertas', async (req, res) => {
    const agora = Date.now();
    if (cacheAlertas.data && (agora - cacheAlertas.lastUpdate < ALERTAS_TTL)) return res.json(cacheAlertas.data);
    try {
...
app.get('/api/clima-clique', async (req, res) => {
    ...
    const key = `${lat}|${lon}`;
    const agora = Date.now();
    if (cacheClima.has(key) && (agora - cacheClima.get(key).last < CLIMA_TTL)) return res.json(cacheClima.get(key).data);
    try {
        const [clima, ar, geo] = await Promise.allSettled([
            axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,weather_code,pressure_msl,wind_speed_10m&daily=temperature_2m_max&forecast_days=14&timezone=auto`),
