...
app.get('/api/clima-clique', async (req, res) => {
    ...
    try {
        const [clima, geo, pol, meteo] = await Promise.all([
            ...
            axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=cloud_cover_low,cloud_cover_mid,cloud_cover_high,cloud_base,precipitation_probability,weather_code&forecast_days=7`)
        ]);

        // Mapeamos os dados para o formato que o seu Index já usa + novos dados
        res.json({ 
            clima: {
                current: {
                    ...
                    rain: clima.data.rain
                }
            }, 
            geo: geo.data, 
            poluicao: pol.data,
            extra: meteo.data.hourly
        });
    } catch (e) { res.status(500).json({ erro: "Erro na API" }); }
});
