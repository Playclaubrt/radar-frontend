...
app.get('/api/alertas', async (req, res) => {
    ...
    try {
        const [resInmet, resNoaa] = await Promise.allSettled([
            ...
            axios.get('https://api.weather.gov/alerts/active', { headers: { 'User-Agent': 'Cloudwindz' } })
        ]);
...
app.get('/api/clima-clique', async (req, res) => {
    ...
    if (cacheClimaLocal.has(key) && (Date.now() - cacheClimaLocal.get(key).last < 600000)) return res.json(cacheClimaLocal.get(key).data);
    try {
        ...
        cacheClimaLocal.set(key, { data: total, last: Date.now() });
        res.json(total);
    } catch (e) { res.status(500).send("Erro"); }
});

app.listen(PORT, () => console.log("Cloudwindz Online"));
